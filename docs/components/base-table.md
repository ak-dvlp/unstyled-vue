...
В компоненте используется обобщённый тип `T` который является объектом (`Record<string, any>`) и описывает структуру одной строки данных. Он связывает тип элементов массива `rows` с ключами динамических слотов для ячеек и конфигурацией колонок.
